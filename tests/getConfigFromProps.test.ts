...
} for the diff Props, they are should not be the same`, () => {
...
test(`Test ${getConfigFromProps.name}  with MetaData`, () => {
  const props: RdpProps<any> = {
    config: {
      customer: { required: false, metaData: true }
    }
  };
  ...
  expect(config.customer.metaData).toBeDefined();
  expect(typeof config.customer.metaData).toBeDefined();
});
