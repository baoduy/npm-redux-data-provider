 ...
 */
export default memoize(<TConfig extends RdpFinalConfig<any>, S>(config: TConfig) => (store: S):
  | RdpConfigStore<TConfig>
  | undefined => {
  const results = {};
  ...
  Object.keys(config).forEach(k => {
    ...
    const tr = cfg.transform && typeof cfg.transform === "function" ? cfg.transform(result) : result;
